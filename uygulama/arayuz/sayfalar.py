...
class AdminPanelPage(QWidget):
    ...
    def _build(self):
        ...
        setup_table(pt)
        pt.setModel(SimpleTableModel(
            ["Kod", "Ad", "Aktif", "Alan Sayısı"],
            [["PRD-001", "Klima Sistemi", "✓", "8"],
             ["PRD-002", "Havalandırma Ünitesi", "✓", "6"],
             ["PRD-003", "Isı Pompası", "✓", "10"]]))
        pl.addWidget(pt)
        pbr = QHBoxLayout()
        ae = QPushButton("+ Ürün Ekle")
        ae.setObjectName("primary")
        pbr.addWidget(ae)
        pbr.addWidget(QPushButton("Düzenle"))
        de = QPushButton("Deaktif Et")
        de.setObjectName("danger")
        pbr.addWidget(de)
        pbr.addStretch()
        pl.addLayout(pbr)
        tabs.addTab(pw, "Ürünler")

        # Kullanıcılar
        uw = QWidget()
        ul = QVBoxLayout(uw)
        ul.setContentsMargins(16, 16, 16, 16)
        ut = QTableView()
        setup_table(ut)
        ut.setModel(SimpleTableModel(
            ["Kullanıcı Adı", "Rol", "Aktif"],
            [["admin", "Admin", "✓"],
             ["editor1", "Editor", "✓"],
             ["editor2", "Editor", "✓"]]))
        ul.addWidget(ut)
        ubr = QHBoxLayout()
        ubr.addWidget(QPushButton("Rol Değiştir"))
        du = QPushButton("Deaktif Et")
        du.setObjectName("danger")
        ubr.addWidget(du)
        ubr.addStretch()
        ul.addLayout(ubr)
        tabs.addTab(uw, "Kullanıcılar")
